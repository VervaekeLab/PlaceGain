function [VeloMatrix,MeanVeloBin] = plotHeatBinVeloPlusBin(LVDATA,Ymax) 
...
SpentInBin = LVDATA.SampleSpentInBinPlusBins;
...
for i = 1:1:TRNuPlot  % rows are trials
    for j = 1:1:BinNu  % columns (distance bins)  
        VeloMatrix(i,j) = mean(Velocity(EnterIntoBin(i,j):(EnterIntoBin(i,j)+SpentInBin(i,j)-1)));
    end
end
...
%PLOT FIGURE
figure('Color','white'); 
imagesc(1:1.9625:157,1:TRNuPlot,VeloMatrix); %(1:number of bins;1:number of trials)   imagesc(1:CR,1:(TR-1),(BinLick))
j = colorbar;
colormap(jet);
