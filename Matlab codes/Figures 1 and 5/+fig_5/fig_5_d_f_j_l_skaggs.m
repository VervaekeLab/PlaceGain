...
load_skaggs_dirct  = '/Users/annachristinagarvert/UIO Physiology Dropbox Dropbox/Lab Data/Ann Christin Garvert/Nora/Paper/Figure5/analysis/skaggs_5cm';
...
for i = 1:length(sessionInfo)
    ...
    off_skaggs_all_single   = [];
    opto_skaggs_all_single  = [];
    after_skaggs_all_single = [];
    
    experiment = sessionInfo(i);
    
    for j = 1:length(experiment.sessions)
         
        load(fullfile(load_skaggs_dirct,experiment.type,experiment.sessions{j},'off_skaggs.mat'))
        load(fullfile(load_skaggs_dirct,experiment.type,experiment.sessions{j},'after_skaggs.mat'))
        load(fullfile(load_skaggs_dirct,experiment.type,experiment.sessions{j},'opto_skaggs.mat'))
        

        off_skaggs_all_single   = [off_skaggs_all_single, off_skaggs];
        opto_skaggs_all_single  = [opto_skaggs_all_single,opto_skaggs];
        after_skaggs_all_single = [after_skaggs_all_single, after_skaggs];
        
        off_skaggs_all(j)   = nanmedian(off_skaggs);
        opto_skaggs_all(j)  = nanmedian(opto_skaggs);
        after_skaggs_all(j) = nanmedian(after_skaggs);
...
%% uncomment this:
    plot([off_skaggs_all',opto_skaggs_all',after_skaggs_all']','k','LineWidth',1)
    hold on
    plot(1:3,[nanmean(off_skaggs_all),nanmean(opto_skaggs_all ),nanmean(after_skaggs_all)],'r','LineWidth',2)
    scatter(2,nanmean(opto_skaggs_all ),50,'filled','r')
    scatter(3,nanmean(after_skaggs_all ),50,'filled','r')
    scatter(1,nanmean(off_skaggs_all),50,'filled','r'); 
    
    scatter(1*ones(length(off_skaggs_all),1),off_skaggs_all',60,'k')
    scatter(2*ones(length(opto_skaggs_all),1),opto_skaggs_all',60,'k')
    scatter(3*ones(length(after_skaggs_all),1),after_skaggs_all',60,'k')
    errorbar([1,2,3],[nanmean(off_skaggs_all),nanmean(opto_skaggs_all),nanmean(after_skaggs_all)],...
        [nanstd(off_skaggs_all)/sqrt(length(off_skaggs_all)),nanstd(opto_skaggs_all)/sqrt(length(off_skaggs_all)),nanstd(after_skaggs_all)/sqrt(length(off_skaggs_all))],'LineStyle','none','LineWidth',2,'Color','r')
   %%
    xticklabels({ 'off', 'opto','after'})
     xticks(gca,[1:3])
     ylabel('Skaggs SI (Bits/s)')
    ax = gca;
    ax.FontSize = 12;
    ax.FontName = 'Arial';
    ax.XColor = [0 0 0];
    ...
    pval(1) = signrank(opto_skaggs_all ,off_skaggs_all ,'tail',experiment.significance);
    pval(2) = signrank(opto_skaggs_all ,after_skaggs_all ,'tail',experiment.significance);
    pval(3) = signrank(after_skaggs_all ,off_skaggs_all);
     pval
    legend('off')
    groups={[1,2],[2,3],[1,3]}; %,[2,3],[1,3]
    sigstar(groups,pval);
    ax.XColor = [0 0 0];
    ax.YColor = [0 0 0];
    ax.XLabel.Color = [0 0 0];
