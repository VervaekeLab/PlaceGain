...
%     [optoMean, optoSEM] = calculateMeanAndSEM(optoMean, experiment.mouse);
    ...
    col = cbrewer('qual','Set3',length(unique(experiment.mouse))+1);col(2,:) = [];
%     fig = makeColMiceFig(offMean,optoMean,afterMean,offSEM,optoSEM,afterSEM,col,pval);
    fig = makeColFig(offMean,optoMean,afterMean,experiment.mouse,col,pval);
    
    fprintf(strcat(sessionInfo(i).type, ' opto-off decoding error is %.2f +/- %.2f cm (mean +/- sem across sessions). \n'),nanmean(offMean),nanstd(offMean)/sqrt(length(offMean)))
    fprintf(strcat(sessionInfo(i).type, ' opto-on decoding error is %.2f +/- %.2f cm (mean +/- sem across sessions). \n'),nanmean(optoMean),nanstd(optoMean)/sqrt(length(offMean)))
    fprintf(strcat(sessionInfo(i).type, ' after-on decoding error is %.2f +/- %.2f cm (mean +/- sem across sessions). \n'),nanmean(afterMean),nanstd(afterMean)/sqrt(length(offMean)))

    fprintf(strcat(sessionInfo(i).type, ': p-value opto-off vs opto-on is %.2f. \n'),pval(1))
...
function fig = makeColFig(offMean,optoMean,afterMean,animals,col, pval)
...
bar(1:3,nanmean([offMean',optoMean',afterMean']))

hold on
for k = 1:length(offMean)
    scatter(1:3,[offMean(k),optoMean(k),afterMean(k)],120,col(animals(k),:),'filled'); hold on
    plot(1:3,[offMean(k),optoMean(k),afterMean(k)],'color',col(animals(k),:), 'LineWidth', 3);
end

%hold on
errorbar(1:3,nanmean([offMean',optoMean',afterMean']),nanstd([offMean',optoMean',afterMean'])./sqrt(length(offMean)),'color',[0 0 0], 'LineWidth', 4)
scatter(1:3,nanmean([offMean',optoMean',afterMean']),120, [0 0 0],'filled'); hold on
plot(1:3,nanmean([offMean',optoMean',afterMean']),'color',[0 0 0], 'LineWidth', 4);
